...
$stmt = $conn->prepare("INSERT INTO Gift (username, password) VALUES (?, ?)");
...
$stmt = $conn->prepare("DELETE FROM Gift WHERE id = ?");
...
$stmt = $conn->prepare("UPDATE Gift SET username = ?, password = ? WHERE id = ?");
...
$result = $conn->query("SELECT * FROM Gift");
